...
### UAV IS NOT GOING IN THE RIGHT DIRECTION WHEN IT CALCULATES THE BEST SIGNAL

### ALTERNATIVELY - IT IS LATE IN REALIZING THE BEST SIGNAL AND USING THE WRONG PTS TO SEND IT "TO THE BEST SIGNAL"

### I BELIEVE THE ERROR IS IN THE LOGIC AND NOT IN THE FUNCTION CALCULATING THE DIRECTION.  BUT I COULD BE WRONG.  FOR THE FIRST TIME ;)
...
class Controller:
	...
	def __init__(self): # This initializes all the global variables
		...
		self.min_signal = -80 # Minimum signal strength needed to decide if a signal is "found"
		#self.min_signal = -90 # Minimum signal strength needed to decide if a signal is "found"
		self.max_signal = -67 # Minimum signal strength needed to decide if a signal is "found"
	...
	def circle_search(self, x_curr, y_curr):
		...
		if x_dist <= 0.5 and len(self.circle_search_pt_list) != 1:
	...
	##### Signal Compare Function: #####
	# This function compares the last known signal to the current signal to determine the new strategy and new control point:
	def signal_compare(self, x_new_temp1, y_new_temp1, x_curr, y_curr, s_curr):
		if s_curr >= self.s_last:
			self.hold_strat = 1
			x_curr_new_temp, y_curr_new_temp = x_new_temp1, y_new_temp1
		else:
			if self.min_signal < s_curr:
				if self.curr_strat == 2:
					if self.circle_flag == 1:
						...
						self.hold_strat = 1
						self.curr_strat = 1
						x_new_temp33, y_new_temp33 = self.global_search(x_curr, y_curr)
						x_curr_new_temp, y_curr_new_temp = x_new_temp33, y_new_temp33
				elif self.curr_strat == 3:
					self.hold_strat = 1
					self.curr_strat = 2
					self.circle_start_pt = (x_curr, y_curr)
					self.circle_search_pt_list = self.define_circle_pts(self.circle_start_pt, self.x_last, self.y_last, x_curr, y_curr) # If I'm switching strategies, I will generate a new set of circle pts
					x_new_temp33, y_new_temp33 = self.circle_search(x_curr, y_curr)

					x_curr_new_temp, y_curr_new_temp = x_new_temp33, y_new_temp33
			else:
				self.hold_strat = 0
				x_curr_new_temp, y_curr_new_temp = self.x_last, self.y_last

		#print "Signal Comparison Function"
		return x_curr_new_temp, y_curr_new_temp
