...
{
   ASSERT_NOT_NULL(kalman);
   ASSERT_NOT_NULL(kalman->filter);
   ASSERT_NOT_NULL(kalman->uk);
   ASSERT_NOT_NULL(kalman->zk);

   cvReleaseKalman(&kalman->filter);
   cvReleaseMat(&kalman->uk);
   cvReleaseMat(&kalman->zk);
   free(kalman);
}
