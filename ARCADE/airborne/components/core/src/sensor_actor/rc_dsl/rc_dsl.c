...
rc_dsl_t *rc_dsl_create(void)
{
   ...
   pthread_mutex_init(&dsl->mutex, NULL);
   return dsl;
}
...
out:
   pthread_mutex_unlock(&dsl->mutex);
   return status;
}
...
int rc_dsl_signal_valid(rc_dsl_t *dsl)
{
   int valid;
   pthread_mutex_lock(&dsl->mutex);
   valid = dsl->RSSI > RC_DSL_VALID_SIGNAL;
   pthread_mutex_unlock(&dsl->mutex);
   return valid;
}
